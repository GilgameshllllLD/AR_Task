...
Shader "Hidden/EdgeDetect" { 
	...
	{	
		...
		float factor = max(0.5 - centerDepth, 0);

		float2 uvDist = (factor * _SampleDistance) * _MainTex_TexelSize.xy;
		...
		float4 original = lerp(tex2D(_MainTex, i.uv[0].xy), _BgColor, _BgFade);

		Sobel = 1.0 - pow(saturate(Sobel), _Exponent);
		if (Sobel < 0.01) {
			float4 brighter = original += float4(0.2, 0.2, 0.2, 0.0);
			return lerp(original, brighter, factor);
		} else {
			return original;
		}
	}
